    ...
    class Coordinator: NSObject, CLLocationManagerDelegate {
        ...
        func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
            ...
            if let lastLocation = lastLocation {
                ...
                parent.runningData.distance += distanceDelta
                
                // 거리 출력
                print("현재 이동 거리 (km): \(parent.runningData.distance)")
                ...
                parent.runningData.pace = String(format: "%02d:%02d", minutes, seconds)
                
                // 페이스 출력
                print("현재 페이스 (분/킬로미터): \(parent.runningData.pace)")
                
                // 칼로리 계산 개선
                let weight = 70.0 // 사용자 체중 (kg) - 실제로는 사용자 입력값을 사용해야 함
                let met = 7.0 // 달리기의 MET 값 (보통 6-10 사이, 속도에 따라 다름)
                let timeInHours = elapsedTime / 60.0 // 분 단위의 경과 시간을 시간 단위로 변환
                let caloriesBurned = met * weight * timeInHours
                
                parent.runningData.calories += Int(caloriesBurned)
                
                // 칼로리 출력
                print("현재 칼로리 소모량 (kcal): \(parent.runningData.calories)")
            }
